...
const response = await axios.post(`${EXPO_PUBLIC_APP_URL}api/auth/login`, data);
...
const Register = async (userData:IRegister) => {
    const response = await axios.post(`${EXPO_PUBLIC_APP_URL}api/auth/register`, userData)
    return response.data
}
...
const response = await axios.post(`${EXPO_PUBLIC_APP_URL}api/auth/settings`,obj)
return response.data
}
const authService ={
    Login,
